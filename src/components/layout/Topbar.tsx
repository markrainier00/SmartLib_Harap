...
export default function Topbar({ isSidebarOpen, toggleSidebar }: TopbarProps) {
  ...
  const handleInformationChange = async (e: React.FormEvent) => {
    ...
    ( role !== "Student" ||
      ( infoForm.newDept === department && infoForm.newProg === program && infoForm.newYear === year )
    );
    ...
    try {
      const payload = {
        school_ID: school_id, email: infoForm.newEmail,
        ...(role === "Student" && { department: infoForm.newDept, program: infoForm.newProg, year: infoForm.newYear }),
      };
      ...
      setInfoForm_({ newEmail: email, ...(role === "Student" && { newDept: department, newProg: program, newYear: year }) });
